...
class OzkPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('ozk.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `ozk`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM ozk WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO ozk (date, title, link, content)"
